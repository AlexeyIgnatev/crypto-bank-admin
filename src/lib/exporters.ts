...
async function exportTxt<T>(opts: ExportOptions<T>): Promise<void> {
  ...
  if (opts.summary?.length) {
    lines.push('');
    lines.push('Summary:');
    for (const item of opts.summary) lines.push(`- ${item.label}: ${item.value}`);
  }
  lines.push('');
  lines.push(opts.columns.map((c) => c.header).join('\t'));
  opts.rows.forEach((row, index) => {
    const cells = opts.columns.map((c) => String(normalizeCell(c.getValue(row, index))));
...
async function exportExcel<T>(opts: ExportOptions<T>): Promise<void> {
  ...
  if (opts.periodLabel) summaryRows.push({ Metric: 'Period', Value: opts.periodLabel });
...
async function exportPdf<T>(opts: ExportOptions<T>): Promise<void> {
  ...
  opts.rows.forEach((row, index) => {
    body.push(opts.columns.map((c) => String(normalizeCell(c.getValue(row, index)))));
  });
  ...
  const docDefinition: any = {
    ...
    content: [
      ...
      ...(summaryText.length ? [{ text: summaryText.join('\n'), margin: [0, 0, 0, 8] }] : []),
      {
        table: {
          headerRows: 1,
          body,
  ...
  pdfMake.createPdf(docDefinition).download(stampFileName(opts.fileBaseName, 'pdf'));
}
