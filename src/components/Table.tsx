...
export default function Table({
  ...
  const [dateTo, setDateTo] = useState<string | undefined>(() => nowIso());
  ...
  function currencySummary(rows: Transaction[]): string {
    ...
    for (const row of rows)
      sums.set(
        row.currency,
        (sums.get(row.currency) || 0) + Number(row.amount || 0),
      );
    return Array.from(sums.entries())
      .sort((a, b) => a[0].localeCompare(b[0]))
      .map(([currency, sum]) => `${currency}: ${formatAmount6(sum)}`)
      .join("; ");
  }
      ...
      await exportRows<Transaction>({
        ...
        columns: [
          ...
          { header: "Статус", getValue: (row) => statusLabel(row.status) },
          {
            ...
            getValue: (row) => rejectReason(row),
          },
          {
            header: "Дата",
            getValue: (row) => new Date(row.createdAt).toLocaleString(),
          },
          { header: "Сумма", getValue: (row) => formatAmount6(row.amount) },
          { header: "Валюта", getValue: (row) => row.currency },
          {
            header: "Комиссия",
            getValue: (row) => formatAmount2(Number(row.feeAmount || 0)),
          },
          { header: "Отправитель", getValue: (row) => row.sender },
          {
            header: "ID отправителя ABS",
            getValue: (row) => row.senderAbsId || row.senderCustomerId || "—",
          },
          { header: "Получатель", getValue: (row) => row.recipient },
          {
            header: "ID получателя ABS",
            getValue: (row) =>
              row.recipientAbsId || row.recipientCustomerId || "—",
          },
          {
    ...
    <div className="flex-1 min-h-0 flex flex-col rounded-xl border border-black/10 dark:border-white/10 overflow-hidden card shadow-sm mb-4">
      ...
      <div className="shrink-0 px-3 py-2 border-b border-black/10 dark:border-white/10 flex items-center justify-between gap-2 bg-[var(--card)]">
        <div className="text-sm text-muted">
          Период реестра: {formatPeriodLabel(dateFrom, dateTo)}
        </div>
            ...
            <tr>
              ...
              <Th>
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ref={currencyDD.btnRef}
                    className="hdr-chip"
                    aria-label="Фильтр"
                    onClick={(e) => {
                      e.stopPropagation();
                      currencyDD.setOpen((o) => !o);
                    }}
                  >
                    <span className="chev">v</span>
                  </button>
              ...
              <Th>
                <div className="flex items-center gap-1">
                  <span className="px-1">Комиссия</span>
                </div>
              </Th>
              <Th>
                <div className="flex items-center gap-1">
              ...
              <Th>
                <div className="flex items-center gap-1">
                  <span className="px-1">ID отправителя ABS</span>
                </div>
              </Th>
              <Th>
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ref={recipientDD.btnRef}
                    className="hdr-chip"
                    aria-label="Фильтр"
                    onClick={(e) => {
                      e.stopPropagation();
                      recipientDD.setOpen((o) => !o);
                    }}
                  >
                    <span className="chev">v</span>
                  </button>
              ...
              <Th>
                <div className="flex items-center gap-1">
                  <span className="px-1">ID получателя ABS</span>
                </div>
              </Th>
              <Th>
                <div className="flex items-center gap-1">
      ...
      <div
        ref={containerRef}
        className="table-scroll flex-1 min-h-0 overflow-y-auto overflow-x-auto [overscroll-behavior:contain] bg-[var(--card)] pb-3"
      >
        <table className="w-full text-sm table-fixed">
          <colgroup>
            <col className="w-[72px]" />
                  ...
                  {vItems.map((vRow) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td
                          className="px-4 py-3 font-mono truncate"
                          title={t.id}
                        >
                          {t.id}
                        </td>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          {formatAmount6(t.amount)}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap">
                          {t.currency}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap">
                          {formatAmount2(Number(t.feeAmount || 0))}
                        </td>
                        <td className="px-4 py-3 truncate" title={t.sender}>
                          {t.sender}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap">
                          {t.senderAbsId || t.senderCustomerId || "—"}
                        </td>
                        <td className="px-4 py-3 truncate" title={t.recipient}>
                          {t.recipient}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap">
                          {t.recipientAbsId || t.recipientCustomerId || "—"}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap">
                          {t.clientAbsId ||
                            t.senderCustomerId ||
      ...
      {idDD.open &&
        createPortal(
          <HeaderDropdown
            pos={idDD.pos}
            onClose={() => idDD.setOpen(false)}
            portalRef={idDD.panelRef}
          >
          ...
          >
            <div className="header-dd p-2">
                ...
                return (
                  ...
                  >
                    <input
                      ...
                      checked={checked}
                      onChange={() => {
                        const n = new Set(statusSet);
                        checked ? n.delete(s) : n.add(s);
                        setStatusSet(n);
                      }}
                    />
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                ...
                <button
                  className="btn btn-success w-full h-9"
                  onClick={() => statusDD.setOpen(false)}
                >
                  Сохранить
                </button>
              </div>
      ...
      {amountDD.open &&
        createPortal(
          <HeaderDropdown
            pos={amountDD.pos}
            onClose={() => amountDD.setOpen(false)}
            portalRef={amountDD.panelRef}
          >
            <div className="header-dd p-2 w-[260px]">
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                ...
                <button
                  className="btn btn-success w-full h-9"
                  onClick={() => amountDD.setOpen(false)}
                >
                  Сохранить
                </button>
              </div>
      ...
      {currencyDD.open &&
        createPortal(
          <HeaderDropdown
            pos={currencyDD.pos}
            onClose={() => currencyDD.setOpen(false)}
            portalRef={currencyDD.panelRef}
          >
            <div className="header-dd p-2">
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                <button
                  className="btn btn-danger w-full h-9"
                  onClick={() => setCurrencySet(new Set())}
                >
                  Сбросить
                </button>
                <button
                  className="btn btn-success w-full h-9"
                  onClick={() => currencyDD.setOpen(false)}
                >
                  Сохранить
                </button>
              </div>
      ...
      {senderDD.open &&
        createPortal(
          <HeaderDropdown
            pos={senderDD.pos}
            onClose={() => senderDD.setOpen(false)}
            portalRef={senderDD.panelRef}
          >
            <div className="header-dd p-2 w-[260px]">
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                ...
                <button
                  className="btn btn-success w-full h-9"
                  onClick={() => senderDD.setOpen(false)}
                >
                  Сохранить
                </button>
              </div>
      ...
      {recipientDD.open &&
        createPortal(
          <HeaderDropdown
            pos={recipientDD.pos}
            onClose={() => recipientDD.setOpen(false)}
            portalRef={recipientDD.panelRef}
          >
            <div className="header-dd p-2 w-[260px]">
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                <button
                  className="btn btn-danger w-full h-9"
                  onClick={() => setRecipientQ("")}
                >
                  Сбросить
                </button>
                <button
                  className="btn btn-success w-full h-9"
                  onClick={() => recipientDD.setOpen(false)}
                >
                  Сохранить
                </button>
              </div>
...
function SortIcon({ active, dir }: { active: boolean; dir: SortDir }) {
  return (
    <span
      className={`inline-block text-white/90 ${active ? "opacity-100" : "opacity-50"}`}
      style={{ width: 12 }}
      aria-hidden
    >
      {dir === "asc" ? "↑" : "↓"}
