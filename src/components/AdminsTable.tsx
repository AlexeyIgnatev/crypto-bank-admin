...
export default function AdminsTable({
  ...
  const [roleSet, setRoleSet] = useState<Set<string>>(
    new Set((filters.roles || []).map(roleKeyToLabel)),
  );
  ...
  useEffect(() => {
    ...
    filters.firstNameQuery,
    filters.lastNameQuery,
    filters.emailQuery,
    filters.createdFrom,
    filters.createdTo,
  ...
  }, [firstQ, lastQ, loginQ, dateFrom, dateTo, roleSet]);
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col rounded-xl border border-black/10 dark:border-white/10 overflow-hidden card shadow-sm">
      ...
      <div
        ref={containerRef}
        className="table-scroll flex-1 min-h-0 overflow-y-auto overflow-x-auto [overscroll-behavior:contain] bg-[var(--card)]"
      >
        <table className="w-full text-sm table-fixed">
          <colgroup>
            <col className="w-[72px]" />
                  ...
                  {items.map((v) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 truncate" title={a.login}>
                          {a.login}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap">
                          <span
      ...
      {firstDD.open && (
        <HeaderDropdown
          pos={firstDD.pos}
          onClose={() => firstDD.setOpen(false)}
          portalRef={firstDD.panelRef}
        >
          <div className="header-dd p-2 w-[260px]">
            ...
            <div className="mt-2 grid grid-cols-2 gap-2">
              ...
              <button
                ...
                onClick={() => {
                  onChangeFilters({ firstNameQuery: firstQ || undefined });
                  firstDD.setOpen(false);
                }}
              >
                Сохранить
              </button>
      ...
      {lastDD.open && (
        <HeaderDropdown
          pos={lastDD.pos}
          onClose={() => lastDD.setOpen(false)}
          portalRef={lastDD.panelRef}
        >
          <div className="header-dd p-2 w-[260px]">
            <div className="text-sm mb-2 font-medium">Фамилия</div>
            <input
              className="ui-input w-full"
              placeholder="Фамилия содержит"
              value={lastQ}
              onChange={(e) => setLastQ(e.target.value)}
            />
            <div className="mt-2 grid grid-cols-2 gap-2">
              <button
                className="btn btn-danger w-full h-9"
              ...
              <button
                className="btn btn-success w-full h-9"
                onClick={() => {
                  onChangeFilters({ lastNameQuery: lastQ || undefined });
                  lastDD.setOpen(false);
                }}
              >
                Сохранить
              </button>
      ...
      {loginDD.open && (
        <HeaderDropdown
          pos={loginDD.pos}
          onClose={() => loginDD.setOpen(false)}
          portalRef={loginDD.panelRef}
        >
          <div className="header-dd p-2 w-[260px]">
            ...
            <div className="mt-2 grid grid-cols-2 gap-2">
              ...
              <button
                className="btn btn-success w-full h-9"
                onClick={() => {
                  onChangeFilters({ emailQuery: loginQ || undefined });
                  loginDD.setOpen(false);
                }}
              >
                Сохранить
              </button>
      ...
      {roleDD.open && (
        <HeaderDropdown
          pos={roleDD.pos}
          onClose={() => roleDD.setOpen(false)}
          portalRef={roleDD.panelRef}
        >
          <div className="header-dd p-2 w-[260px]">
            ...
            <div className="mt-2 grid grid-cols-2 gap-2">
              ...
              <button
                ...
                onClick={() => {
                  onChangeFilters({
                    roles: Array.from(roleSet).map(roleLabelToKey),
                  });
          ...
          <div className="header-dd p-2 w-[260px]">
            ...
            <div className="mt-2 grid grid-cols-2 gap-2">
              ...
              <button
                className="btn btn-success w-full h-9"
                onClick={() => {
                  onChangeFilters({ createdFrom: dateFrom, createdTo: dateTo });
