...
export default function CasesTable({
  ...
  onOpen: (t: AntiFraudCaseItem) => void;
  refreshToken?: number;
}) {
  ...
  const [statusSet, setStatusSet] = useState<Set<AntiFraudCaseStatus>>(
    new Set(),
  );
                  ...
                  {vItems.map((vRow) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          {formatAmount6(item.amount)}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap">
                          {item.currency}
                        </td>
                        <td className="px-4 py-3 truncate" title={item.sender}>
                          {item.sender}
                        </td>
                        <td
      ...
      {statusDD.open && (
        <DropdownPanel state={statusDD} title="Статус">
          <div className="space-y-1">
            {(["OPEN", "APPROVED", "REJECTED"] as AntiFraudCaseStatus[]).map(
              (s) => (
      ...
      {dateDD.open && (
        <DropdownPanel state={dateDD} title="Дата">
          <div className="grid grid-cols-1 gap-2">
            <Flatpickr
              className="ui-input w-full"
              options={{ locale: Russian, dateFormat: "Y-m-d" }}
              placeholder="с"
              value={dateFrom as any}
              onChange={(d) =>
                setDateFrom(d?.[0] ? formatDate(d[0]) : undefined)
              }
            />
            <Flatpickr
              className="ui-input w-full"
              ...
              placeholder="по"
              value={dateTo as any}
              onChange={(d) => setDateTo(d?.[0] ? formatDate(d[0]) : undefined)}
            />
          </div>
        </DropdownPanel>
      )}
        ...
        <DropdownPanel state={amountDD} title="Сумма">
          <div className="grid grid-cols-2 gap-2">
            <input
              ...
              placeholder="от"
              value={minAmount ?? ""}
              onChange={(e) => setMinAmount(num(e.target.value))}
            />
            <input
              className="ui-input"
              type="number"
              ...
              placeholder="до"
              value={maxAmount ?? ""}
              onChange={(e) => setMaxAmount(num(e.target.value))}
            />
          </div>
        </DropdownPanel>
      )}
...
function Th({
  ...
  return (
    <th
      className={`px-4 py-3 text-left text-xs select-none ${onClick ? "cursor-pointer" : ""}`}
      onClick={onClick}
    >
      <div
        className={`inline-flex items-center gap-1 rounded px-1 ${active ? "bg-black/10" : ""}`}
      >
        {children}
      </div>
    </th>
...
function SortIcon({ active, dir }: { active?: boolean; dir?: SortDir }) {
  return (
    <svg
      ...
      viewBox="0 0 24 24"
      aria-hidden="true"
      className={`opacity-80 ${active ? "" : "opacity-40"}`}
    >
      <path
        d="M7 14l5-5 5 5z"
        fill="currentColor"
        transform={dir === "asc" ? "rotate(180 12 12)" : ""}
      />
    </svg>
  );
}
function DropdownPanel({
  ...
  return (
    <div
      ref={state.panelRef}
      className="fixed z-50 card border border-soft rounded-xl shadow-xl p-3"
      style={{
        top: state.pos.top,
        left: state.pos.left,
    ...
    >
      ...
      <div className="pt-2 text-right">
        <button className="btn h-8" onClick={() => state.setOpen(false)}>
          Закрыть
        </button>
      </div>
    </div>
