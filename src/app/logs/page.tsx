...
function SortIcon({ active, dir }: { active: boolean; dir: "asc" | "desc" }) {
  return (
    <span
      className={`inline-block ${active ? "opacity-100" : "opacity-50"}`}
      style={{ width: 12 }}
      aria-hidden
    >
      {dir === "asc" ? "↑" : "↓"}
...
export default function AdminLogsPage() {
  ...
  const [sortKey, setSortKey] = useState<"createdAt" | "admin_id" | "action">(
    "createdAt",
  );
  ...
  async function fetchPage(pageOffset: number, replace: boolean) {
    ...
    try {
      ...
      const res = await fetch(`/api/audit/admin-actions?${params.toString()}`, {
        cache: "no-store",
      });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="text-sm text-muted">
            Показаны последние события с возможностью подгрузки
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0 flex flex-col rounded-xl border border-soft card shadow-sm overflow-hidden">
              ...
              <tr>
                ...
                <Th onClick={() => toggleSort("admin_id")}>
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      ref={adminIdDD.btnRef}
                      className="hdr-chip"
                      aria-label="Фильтр"
                      onClick={(e) => {
                        e.stopPropagation();
                        adminIdDD.setOpen((o) => !o);
                      }}
                    >
        ...
        {adminIdDD.open &&
          createPortal(
            <HeaderDropdown
              pos={adminIdDD.pos}
              onClose={() => adminIdDD.setOpen(false)}
              portalRef={adminIdDD.panelRef}
            >
              <div className="header-dd p-2 w-[220px]">
                ...
                <div className="mt-2 grid grid-cols-2 gap-2">
                  <button
                    className="btn btn-danger w-full h-9"
                    onClick={() => setAdminIdFilter("")}
                  >
                    Сбросить
                  </button>
                  <button
                    className="btn btn-success w-full h-9"
                    onClick={() => adminIdDD.setOpen(false)}
                  >
                    Сохранить
                  </button>
                </div>
        ...
        {actionDD.open &&
          createPortal(
            <HeaderDropdown
              pos={actionDD.pos}
              onClose={() => actionDD.setOpen(false)}
              portalRef={actionDD.panelRef}
            >
              <div className="header-dd p-2 w-[320px]">
                ...
                <div className="mt-2 grid grid-cols-2 gap-2">
                  ...
                  <button
                    className="btn btn-success w-full h-9"
                    onClick={() => actionDD.setOpen(false)}
                  >
                    Сохранить
                  </button>
                </div>
              ...
              {items.map((it) => (
                <tr
                  ...
                  onClick={() => {
                    ...
                    setOpen(true);
                    setAdminInfo(null);
                    loadAdminInfo(it.admin_id);
                  }}
                >
                  <td className="px-4 py-3 tabular-nums text-muted">{it.id}</td>
                  <td className="px-4 py-3">{it.admin_id}</td>
                  <td className="px-4 py-3 truncate" title={it.ip}>
                    {truncate(it.ip, 24)}
                  </td>
                  <td className="px-4 py-3 truncate" title={it.action}>
                    {truncate(it.action, 64)}
                  </td>
                  <td
                    className="px-4 py-3 truncate"
                    title={formatDetails(it.details)}
                  >
                    {truncate(formatDetails(it.details), 64)}
                  </td>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Детали события">
        {selected && (
          <div className="space-y-2 text-sm text-fg">
            ...
            <Row
              label="Админ ID"
              value={<span className="font-mono">{selected.admin_id}</span>}
            />
            ...
            <Row
              label="Детали"
              value={
                <pre className="whitespace-pre-wrap break-words text-xs p-2 rounded bg-black/5 dark:bg-white/10">
...
function Th({
  ...
  return (
    <th
      onClick={onClick}
      className="text-left text-xs font-semibold uppercase tracking-wide sticky top-0 px-3 py-2 select-none"
    >
      {children}
    </th>
  );
...
function Td({ children }: { children: React.ReactNode }) {
  return (
    <td className="px-3 py-2 border-t border-soft truncate">{children}</td>
  );
}
