...
export default function AdminsPage() {
  ...
  const [sort, setSort] = useState<{ key: AdminSortKey; dir: SortDir }>({
    key: "createdAt",
    dir: "desc",
  });
  ...
  return (
    <div className="space-y-3 text-sm">
      ...
      <div className="grid grid-cols-3 gap-2 pt-6">
        <button className="btn w-full h-9" onClick={onClose}>
          Закрыть
        </button>
        <button className="btn btn-info w-full h-9" onClick={onEdit}>
          Редактировать
        </button>
        <button className="btn btn-danger w-full h-9" onClick={onDelete}>
          Удалить
        </button>
      </div>
    </div>
        ...
        <button
          ...
          onClick={async () => {
            ...
            try {
              ...
              setErr("Не удалось удалить администратора");
            } finally {
              setSubmitting(false);
            }
          }}
